...
public:
	...
	void swap(LitVec& lits, uint32 sw) {
		literals_.swap(lits);
		w2_ = sw;
	}
...
public:
	 ...
	 * \note The clause must be destroyed using Clause::destroy
	 */ 
	static Clause*  newLearntClause(Solver& s, const LitVec& lits, ConstraintType t, LitVec::size_type secondWatch);
	 ...
	 * \note The clause must be destroyed using Clause::destroy
	 */ 
	static Clause*  newContractedClause(Solver& s, const LitVec& lits, LitVec::size_type secondWatch, LitVec::size_type tailStart);
	 ...
	 */
	uint32 activity() const { return activity_; }
	...
	Literal* begin()  { return lits_; }
	Literal* end()    { return lits_ + size_; }
protected:
	~Clause() {}
private:
	...
	//  * The "active" part contains the literals from the highest decision levels
	...
	//    * sets the size of the clause to the number of literals between lits_[0] and ~X
	...
	mutable uint32 last_;   // where did last search stop and in what direction were we going?
	uint32  activity_;      // clause activity, number of times reason was called
	// The literals of this clause.
	// Layout: l1 l2 ... ln S
	// S is a sentinel literals, s.th:
	//  * isTrue(S) && S.watched()
	// The literals lits_[0] and lits_[1] are the two watched watched literals of the clause
	// and if he clause is unit, lits_[0] is the unit-resulting literal.
	Literal           lits_[0];   
};
