...
bool ModelEnumerator::backtrackFromModel(Solver& s, int *state) {
...
switch(*state) {
  ...
  case 1: {
	   if(s.strategies().satPrePro.get() && s.strategies().satPrePro->hasSymModel() && numModels_ != s.stats.models) {
...
bool RecordEnumerator::doBacktrackFromModel(Solver& s, uint32 bl) {
	...
	if (solution_.size() < 4) {
		...
		Clause* c = Clause::newLearntClause(s, solution_.lits(), Constraint_t::learnt_conflict, solution_.secondWatch());
